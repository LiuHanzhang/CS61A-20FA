...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        outcome = dice()
        sum += outcome
        pigout = pigout or (False if outcome != 1 else True)
    return sum if not pigout else 1
    # END PROBLEM 1
...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    pi = FIRST_101_DIGITS_OF_PI // (10 ** (100 - score))
    # END PROBLEM 2
...
def GCD(a, b):
    """欧几里德算法 https://pic2.zhimg.com/v2-bf7892aa8133840e100ca8eddc09f8d5_b.jpg"""
    while a % b != 0:
        r = a % b
        a = b
        b = r
...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    if (player_score and opponent_score) == 0:
        return False
    return True if GCD(player_score, opponent_score) >= 10 else False
    # END PROBLEM 4a
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        curr = score1 if who else score0
        if curr - last_score > running_high:
            print(curr - last_score, "point(s)! The most yet for Player", who)
        return announce_highest(who, curr, max(curr - last_score, running_high))
    return say
    # END PROBLEM 7
