...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place = self.place
        curr = 0
        while place and place != beehive and (curr != self.min_range):
            place = place.entrance
            curr += 1
        while place and place != beehive and (curr <= self.max_range) and (not place.bees):
            place = place.entrance
            curr += 1
        if not (place and place != beehive and (self.min_range <= curr <= self.max_range)):
            return None
        else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        reflect_amount = amount + (self.damage if self.armor <= amount else 0)
        for bee in self.place.bees[:]:
            bee.reduce_armor(reflect_amount)
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        self.real_queen = False if QueenAnt.has_instantiated else True
        QueenAnt.has_instantiated = True
        self.doubled_ants = []
