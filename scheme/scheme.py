...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # NOTE: Procedure validity must be checked before operands are evaluated
        validate_procedure(operator)
        if isinstance(operator, MacroProcedure):
            return scheme_eval(operator.apply_macro(rest, env), env)
        else: # Lambda or Buildin Procedure
            def scheme_eval_w_env(expr):
                return scheme_eval(expr, env)
            operands = rest.map(scheme_eval_w_env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        lambda_func = do_lambda_form(Pair(target.rest, expressions.rest), env)
        env.define(target.first, lambda_func)
        return target.first
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        name, expr = binding.first, binding.rest
        validate_form(expr, 1, 1)
        names = Pair(name, names) # NOTE: The order of names/values is reversed
        values = Pair(scheme_eval(expr.first, env), values) # So we have to ban duplicate bindings
        bindings = bindings.rest
    validate_formals(names) # NOTE: Ban duplicate bindings
    # END PROBLEM 14
...
def do_define_macro(expressions, env):
    ...
    if not scheme_symbolp(name):
        raise SchemeError("Invalid symbol {0}".format(name))
