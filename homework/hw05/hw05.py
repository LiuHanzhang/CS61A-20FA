...
def inorder(t):
    if len(t.branches) > 2:
        raise Exception
    if len(t.branches) == 2:
        yield from inorder(t.branches[0])
        yield t.label
        yield from inorder(t.branches[1])
        ...
        # Leaf node. Recursion base
        yield t.label

def is_incremental(l):
...
def is_bst(t):
    ...
    "*** YOUR CODE HERE ***"
    try:
        intraverse = list(inorder(t))
        print("DEBUG: ", intraverse)
    except Exception:
        return False
    else:
        return is_incremental(intraverse)
...
def path_yielder(t, value):
    ...
    if t.label == value:
        yield [t.label]
    for b in t.branches:
        yield from [ [t.label] + p for p in path_yielder(b, value)]
