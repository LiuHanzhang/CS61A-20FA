...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_virtual = make_city('Virtual', lat, lon)
    return get_name(city_a) if distance(city_a, city_virtual) <= distance(city_b, city_virtual) else get_name(city_b)
...
def riffle(deck):
    ...
    "*** YOUR CODE HERE ***"
    M = len(deck) // 2
    return [deck[i // 2 + (i % 2) * M] for i in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):
        ...
        return tree(label(t1) + label(t2), [add_trees(b1, b2) for b1, b2 in zip(branches(t1), branches(t2))] + branches(t2)[len(branches(t1)):])
    else:
        return tree(label(t1) + label(t2), [add_trees(b1, b2) for b1, b2 in zip(branches(t1), branches(t2))] + branches(t1)[len(branches(t2)):])
